...
def report_diff(context, a,  b):
    out1 = torch.reshape(a.to(torch.float32).detach(), [-1]).cpu().numpy()  
    out2 = torch.reshape(b.to(torch.float32).detach(), [-1]).cpu().numpy()
    diff = np.abs(out1 - out2)
    args_max = np.argmax(diff)
    print(f"{context}: arg_max {args_max}, max diff {diff[args_max]}, out1 {out1[args_max]} out2 {out2[args_max]}")
    print(f"{context}: diff {diff}")
